...
st.markdown('''<h1 style='text-align: center; color: black;'
            >Предварительная обработка данных часть 2: Сравнение результатов </h1>''', 
...
if step_two:
  ...
  во-вторых, если мы оставим эти столбцы, то можем столкнуться с проблемой **[мультиколлениарноcти](https://www.bigdataschool.ru/wiki/мультиколлинеарность)**, так как часть этой информации уже есть в целевой переменной. 
  Мультиколлинеарность - наличие линейной зависимости между признаками, т.е. когда можно выразить один признак (фичу) через другие, что зачастую приводит к переобучению модели. Это происходит потому, что у нас переизбыток информации. Проверять наличие линейной зависимости будем при помощи определителя матрицы. **[Определитель матрицы](https://ru.onlinemschool.com/math/library/matrix/determinant/)** - это понятие из высшей математики. Нам не очень важно само понятие, нас инетресует одно из его свойств:  
  определитель матрицы равен нулю, если две или несколько строк или столбцов матрицы линейно зависимы. Следовательно, чем определитель матрицы ближе к нулю, тем сильнее мультиколлениарность и ненадежнее результаты. Чем ближе определитель к единице, тем меньше мультиколлениарность.
  Скорее всего, вы не получите просто цифру 0, когда будете делать вычисления определителя матрицы, но получите [экспоненциальную запись числа](https://allcalc.ru/node/1103), как на фото ниже. Это и будет подтверждением наличия мультиколлениарности.""")
  scientific_notation = Image.open('images/scientific_notation.png')
  st.image(scientific_notation)
  st.write("""
  Есть несколько вариантов предобработки. Мы можем просто удалить эти данные, можем изменить тип данных на int64 и оставить (не лучший вариант из-за возможной мультиколлениарности), а можем вытащить дополнительную информация, например, месяц, день недели и время поездки (datetime дает нам такую возможность), а затем уже удалить столбцы начало и конец поездки.
...
if step_three:
  st.write("""
  \n Мы можем сделать еще одно маленькое преобразование: у нас есть индексы и столбец id. Посмотрите на уникальные значения переменной "id". У каждой поездки он свой. По сути, это то же самое, что и индекс. Давайте сделаем так, чтобы колонка id стала нашим индексом.
  """)
...
if step_five:
  ...
  lin_reg.markdown("""
  **[Линейная регрессия](https://neurohive.io/ru/osnovy-data-science/linejnaja-regressija/)** - это модель зависимости целевой переменной от одной или нескольких других переменных(столбцов/фичей).
  ...
  st.write("""На графике снизу черная линия - линейная регрессия, красные квадратики - предсказания линейной регресии, зеленные кружки - реальные значения целевой переменной, пунктиром отмеченна разница каждого предсказания с реальным значением.
  Затем эта разница считается по метрике: берутся все значения разниц, суммируются друг с дургом, а затем мы делим это число на количесвто наблюдений (на фото их 6).
   """)
...
st.write("""
В этом блоке вы можете попробовать свои силы: самостоятельно обработать данные и обучить модель. Для того, чтобы лучше понять, как это все работает, попробуйте выполнить данную лабораторную несколько раз и проанализируйте полученные результаты. Что вы меняли чтобы улучшить модель? А что влияло на ухудшение показателя метрики?
Попробуйте достичь следующих результатов: MAE меньше или равен 465, определитель матрицы больше или равен 0.65.
\nАлгоритм и метрика те же самые - линенйная регрессия и MAE.
...
if step_eight:
  ...
  Идея заключается в том, чтобы посчитать разность долго и широт, а потом вычислить расстояниемежду двумя точками по теореме Пифагора. Но для начала нам нужно правильно перевести долготу и широту в километры, так как их градусная мера имеет неодинаковую шкалу перевода.
...
if step_nine:
  ...
  Посмотрите на 2 графика снизу. Это предсказания алгоритма линейной регресии. Красные кружки - реальные показатели, а пунктиром отмечена разница предсказаний с реальными значениями (ошибки). Как вы помните, любая метрика - среднее значение по всем ошибкам. Алгоритм всегда пытается минимизировать эту среднюю ошибку. 
  На графике слева у нас нет выбросов и алгоритм линеной регресии смог простроить хорошую линию. На графике справа - те же самые данные, но с 3 выбросами. Наш алгоритм теперь учитывает и эти экстримальные значения. Во втором случае наши предсказания будут намного сильнее отличаться от реальных, чем в первом, соотвественно показатель метрики и функция ошибки будут вышее. Именно поэтому от выбросов стараются избавляться.
  """)
  ...
  if show_plots:
    ...
    st.write("""
    Мы выдим, что у нас довольно большие выбросы (настолько большие, что на графике даже не видно сам ящик с усами). Чтобы модель лучше уловила зависимости в данных, от выбросов лучше избавиться. При этом, полностью от выбросов избавляться нельзя. 
    Давайте обозначим границы: отсечем 5 сотых значений сверху и 5 сотых значений снизу. Воспользуемся для этого перцентилями. То есть верхний порог у нас будет 0.995 перцентиль, а нижний 0.005 перцентиль.
    """)

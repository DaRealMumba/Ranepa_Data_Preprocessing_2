...
\n Есть несколько вариантов развития событий. Мы можем просто удалить эти данные, можем изменить тип данных на int64 и оставить, а можем пойти дальше и вытащить дополнительную информация, например, месяц, день недели и время поездки,а затем уже удалить столбцы начало и конец поездки.
...
# info_expander = st.expander('Пояснение')
# info_expander.markdown('Благодаря тому, что мы перевели столбцы начало и конец поездки в формат datetime, мы можем излвечь из них такую информацию')
...
if show_df:
  if get_more_info[0] == 'Да':
    df_2['месяц_поездки'] = pd.DatetimeIndex(df_2['начало_поездки']).month
    df_2['день_поездки'] = pd.DatetimeIndex(df_2['начало_поездки']).day
    df_2['час_начала_поездки'] = pd.DatetimeIndex(df_2['начало_поездки']).hour
